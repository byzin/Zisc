...
inline
LockFreeLockArt<Key, T, Compare>::LockFreeLockArt(pmr::memory_resource* mem_resource) noexcept :
    LockFreeLockArt(1, mem_resource)
{
}
...
template <std::movable Key, MappedValue T, std::invocable<Key, Key> Compare>
inline
LockFreeLockArt<Key, T, Compare>::LockFreeLockArt(const size_type cap,
    ...
    requires std::is_nothrow_constructible_v<ValueT, Args...>
{
  ...
  size_type node_index = invalidId();
  {
    ...
    auto pos = std::lower_bound(list.begin(), list.end(), key, LockFreeLockArt::compare);
    if ((pos == list.end()) || !LockFreeLockArt::equal(*pos, key)) {
      // 
