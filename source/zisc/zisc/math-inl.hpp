...
{
  static_assert(kIsFloat<Float>, "Float isn't floating point type.");
  constexpr auto pi = kPi<Float>;
  constexpr auto half_pi = cast<Float>(0.5) * pi;
  ZISC_ASSERT(isInClosedBounds(x, -pi, pi), "The x is out of range.");
  x = (half_pi < x)  ? pi - x
    : (x < -half_pi) ? -pi - x
    :                  x;
...
Float cos(const Float x) noexcept
{
  ...
  constexpr auto half_pi = cast<Float>(0.5) * pi;
  ZISC_ASSERT(isInClosedBounds(x, -pi, pi), "The x is out of range.");
  return sin((-half_pi < x) ? half_pi - x : -(pi + half_pi) - x);
}
