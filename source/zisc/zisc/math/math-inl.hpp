...
/*!
  ...
  */
template <FloatingPoint Float> inline
constexpr Float Math::copysign(const Float x, const Float y) noexcept
{
  using BitType = BitT<Float>;
  const BitType xi = bit_cast<BitType>(x);
  const BitType yi = bit_cast<BitType>(y);
  constexpr BitType mask = cast<BitType>(1ull << (8 * sizeof(BitType) - 1));
...
/*!
  ...
  */
template <FloatingPoint Float> inline
constexpr Float Math::mulsign(const Float x, const Float y) noexcept
{
  using BitType = BitT<Float>;
  const BitType xi = bit_cast<BitType>(x);
  const BitType yi = bit_cast<BitType>(y);
