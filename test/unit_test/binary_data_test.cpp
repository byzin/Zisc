...
TEST(BinaryDataTest, ReadWriteTest)
{
  ...
  zisc::write(data2, &binary_stream, data2.size());
  ...
  // Test "read"
  {
    const int result1 = zisc::read<zisc::int32>(&binary_stream);
    ASSERT_EQ(data1, result1);
  }
  {
    const std::string result2 = zisc::read<std::string>(&binary_stream, data2.size());
    ASSERT_EQ(data2, result2);
  }
  {
    const TestData result3 = zisc::read<TestData>(&binary_stream);
    for (int i = 0; i < 4; ++i)
