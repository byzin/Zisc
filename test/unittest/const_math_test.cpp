...
struct ConstexprTest
{
  ...
  static constexpr double normal = ::makeNormal(x);
  static_assert((normal == zero) || zisc::isNormal(normal),
                "The normal isn't normal.");

  static constexpr double subnormal = ::makeSubnormal(x);
  ...
  {
    {
      constexpr double result = zisc::constant::sqrt(normal);
      const double expected = std::sqrt(normal);
      EXPECT_DOUBLE_EQ(expected, result)
          << "The result of sqrt(" << normal << ") is wrong.";
    }
